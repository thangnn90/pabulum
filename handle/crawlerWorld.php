...
for ($i = 36; $i <= 36; $i--) {
$Mlink = "http://www.amthuc365.vn/cam-nang-huu-ich/me-va-be.html&page=$i";
...
foreach ($items as $item) {
    ...
    $html_content = file_get_html('http://www.amthuc365.vn/t8587c195/me-va-be/2011/07/' . $link);
    ...
    $query = "SELECT * FROM pabulum where name='$title' AND cate_id=5";
    if ($result_exits = mysqli_query($con, $query)) {
        if (mysqli_num_rows($result_exits) > 0) {
            echo 'Tồn tại tin trong dữ liệu' . "<br>";
        } else {
            $sql = "INSERT INTO pabulum(cate_id,name,img,mota,content,view_number,like_number) VALUES (4,'$title','$img','$decs','$noidung',0,0)";
